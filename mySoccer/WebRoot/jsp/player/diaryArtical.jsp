...
<script type="text/javascript">
	...
	window.onload = function() {
		...
		if(!(id.length===0)) {
			var diary="<%=request.getAttribute("diary")%>"; 
			var diaryId = "${diary.diaryId}";
			$.post("${pageContext.request.contextPath}/findCollection",{'diaryId':diaryId},function(data) {
				if(data == "1" ) {
		...
		var diary="<%=request.getAttribute("diary")%>"; 
		var diaryId = "${diary.diaryId}";
		$.post("${pageContext.request.contextPath}/findCommentsByArticleId",{'diaryId':diaryId},function(data) {
					...
					+'<div class="media-body">'
					  ...
					  if((jsonObj[i].userId == clubId)||(jsonObj[i].userId == coachId)||(jsonObj[i].userId == playerId)||(jsonObj[i].userId == touristId)) {
						html+=' <div class="ibox-tools">'
							+'<a href="javascript:void(0)" onclick="deleteComments(\''+jsonObj[i].commentsId+'\',\''+diaryId+'\')" >删除</a>'
							+'</div>'
					}  
	...
	function change(diaryId) {
		
		var type = document.getElementById('collection').innerText;
		
		if(type == "收藏") {
			$.post("${pageContext.request.contextPath}/addCollection",{'diaryId':diaryId},function(data) {
				document.getElementById('collection').innerText = "取消收藏";
			});
		} else {
			$.post("${pageContext.request.contextPath}/deleteCollection",{'diaryId':diaryId},function(data) {
	...
	function addComments(diaryId) {
		...
		$.post("${pageContext.request.contextPath}/addComments",{'contents':contents,'diaryId':diaryId},function(data) {
			if(data == "1") {
	...
	function deleteComments(commentsId,diaryId) {
            ...
            <div class="col-lg-10 col-lg-offset-1">
                <div class="ibox">
                    <div class="ibox-content"> 
                     ...
                     <c:if test="${not empty existTourist }">
                        <a href="javascript:void(0)" onclick="change('${diary.diaryId}')" id="collection"  class="btn btn-default btn-xs" role="button" >收藏</a> 
                     </c:if>
                     
                     	<!-- 当查看队友日记时没有编辑权限 -->
                     	 <c:if test="${(diary.diaryPlayer.pid==existPlayer.pid)&&(not empty existPlayer) }">
                     		<c:if test="${empty pageFlag }">
	                     		<a href="${pageContext.request.contextPath }/jumpToUpdateDiary?diaryId=${diary.diaryId}">
						...
						<div class="small m-b-xs">
							<c:if test="${not empty pageFlag }">
                          		<span > by </span> <strong><a href="${pageContext.request.contextPath }/getplayerInformation?pageFlag=1&pid=${diary.diaryPlayer.pid}">${ diary.diaryPlayer.pRealName }</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong> <span><i class="fa fa-clock-o"></i>&nbsp;&nbsp;${ fn:substring(diary.diaryDate,0,19) }</span>
						    </c:if>
						    <c:if test="${empty pageFlag }">
                          		<span > by </span> <strong><a href="${pageContext.request.contextPath }/getplayerInformation?pid=${diary.diaryPlayer.pid}">${ diary.diaryPlayer.pRealName }</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong> <span><i class="fa fa-clock-o"></i>&nbsp;&nbsp;${ fn:substring(diary.diaryDate,0,19) }</span>
						    </c:if>
                        </div>
						 <hr>
							${diary.diaryContents }
	                        ...
	                        <div class="input-group">
	                            <input type="text" id="contents" name="contents" placeholder="请在此发表您的评论..." class="input form-control">
	                            	<span class="input-group-btn">
	                                        <button type="button" onclick="addComments('${diary.diaryId}')" class="btn btn btn-info btn-sm">发表</button>
	                                </span>
	                        </div>
	                        <hr>
	                        <br>
