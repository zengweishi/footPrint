  ...
  <body>
		  ...
		  <div style="margin-top: 20px;margin-left: 20px">
		    ...
		    <!-- Indicates a successful or positive action -->
		 		...
		 		<button type="button" value="submit" id="release" onclick="doPost('${pageContext.request.contextPath }/updateDiary')"  name="release" class="btn btn-success">更新日记</button>
 ...
 <%-- <a href="javascript:void(0)"  id="release" onclick="doPost('${pageContext.request.contextPath }/writeCNews')" class="btn btn-primary" role="button" class="btn btn-primary btn-sm" data-toggle="modal"
    ...
    <script type="text/javascript">
      ...
      //配置onfocus函数之后，用户点击富文本区域会触发onfocus函数执行。
		...
		var updateDiary="<%=session.getAttribute("updateDiary")%>"; 
		document.getElementById("diaryTitle").value="${updateDiary.diaryTitle}";
		//document.getElementById("editor").setContent("${cnews.newsContents}"); 
		//$("#editor").text("${cnews.newsContents}");
		editor.txt.html("${updateDiary.diaryContents}");//牛逼啊这个方法，将数据回显到编辑器
		};
		...
		function doPost(url){
		    ...
		    var id = "${updateDiary.diaryId}";
		  
			$.post(url,{'diaryContents':diaryContents,'diaryTitle':diaryTitle,'diaryPermission':diaryPermission,'id':id},function(data) {   
				window.location.href="${pageContext.request.contextPath}/findDiaryArtical?diaryId=${updateDiary.diaryId}";
			});
