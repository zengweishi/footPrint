...
<script type="text/javascript">
	...
	window.onload = function() {
		...
		if(!(id.length===0)) {
			var tnews="<%=request.getAttribute("tnews")%>"; 
			var tnewsId = "${tnews.newsId}";
			$.post("${pageContext.request.contextPath}/findCollection",{'tnewsId':tnewsId},function(data) {
				if(data == "1" ) {
		...
		var tnews="<%=request.getAttribute("tnews")%>"; 
		var tnewsId = "${tnews.newsId}";
		$.post("${pageContext.request.contextPath}/findCommentsByArticleId",{'tnewsId':tnewsId},function(data) {
				...
				html+='<div class="social-feed-box">'
					...
					if((jsonObj[i].userId == clubId)||(jsonObj[i].userId == coachId)||(jsonObj[i].userId == playerId)||(jsonObj[i].userId == touristId)) {
						html+=' <div class="ibox-tools">'
							+'<a href="javascript:void(0)" onclick="deleteComments(\''+jsonObj[i].commentsId+'\',\''+tnewsId+'\')" >删除</a>'
							+'</div>'
					}  
	...
	function change(tnewsId) {
		
		var type = document.getElementById('collection').innerText;
		
		if(type == "收藏") {
			$.post("${pageContext.request.contextPath}/addCollection",{'tnewsId':tnewsId},function(data) {
				document.getElementById('collection').innerText = "取消收藏";
			});
		} else {
			$.post("${pageContext.request.contextPath}/deleteCollection",{'tnewsId':tnewsId},function(data) {
	...
	function addComments(tnewsId) {
		...
		$.post("${pageContext.request.contextPath}/addComments",{'contents':contents,'tnewsId':tnewsId},function(data) {
			if(data == "1") {
	...
	function deleteComments(commentsId,tnewsId) {
                ...
                <div class="ibox">
                    <div class="ibox-content"> 
                     <div class="pull-right">
                     		...
                     		<c:if test="${not empty existTourist }">
                     			<a href="javascript:void(0)" onclick="change('${tnews.newsId}')" id="collection"  class="btn btn-default btn-xs" role="button" >收藏</a> 
                     		</c:if>
                     
                     	
                     	 <c:if test="${(tnews.newsTourist.tId==existTourist.tId)&&(not empty existTourist) }">
                     		<c:if test="${empty pageFlag }">
	                     		<a href="${pageContext.request.contextPath }/jumpToUpdateTNews?newsId=${tnews.newsId}">
                        ...
                        <div class="text-center article-title">
                            <h2>
                                 <strong>   ${tnews.newsTitle }</strong>
                                </h2>
                        </div>
						<div class="small m-b-xs">
							<c:if test="${not empty pageFlag }">	
                         		 <span > by </span> <strong><a href="${pageContext.request.contextPath }/getTouristInformation?pageFlag=1&tId=${tnews.newsTourist.tId}">${ tnews.newsTourist.tName }</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong> <span><i class="fa fa-clock-o"></i>&nbsp;&nbsp;${ fn:substring(tnews.newsDate,0,19) }</span>
							</c:if>
							<c:if test="${empty pageFlag }">	
                         		 <span > by </span> <strong><a href="${pageContext.request.contextPath }/getTouristInformation?tId=${tnews.newsTourist.tId}">${ tnews.newsTourist.tName }</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong> <span><i class="fa fa-clock-o"></i>&nbsp;&nbsp;${ fn:substring(tnews.newsDate,0,19) }</span>
							</c:if>
                        </div>
						 <hr>
							${tnews.newsContents }
	                         ...
	                         <div class="input-group">
	                            <input type="text" id="contents" name="contents" placeholder="请在此发表您的评论..." class="input form-control">
	                            	<span class="input-group-btn">
	                                        <button type="button" onclick="addComments('${tnews.newsId}')" class="btn btn btn-info btn-sm">发表</button>
	                                </span>
	                        </div>
	                        <hr>
	                        <br>
