                    ...
                    <div class="ibox-content">
                        ...
                        <div class="row">
                             <div class="col-md-6">
                                ...
                                <!-- 设置当查看队友日记时没有删除权限 -->
                                <c:if test="${p.newsTourist.tId==existTourist.tId }">
 							 <a href="javascript:void(0)" onclick="deleteTNewsBt('${p.newsId}')"  class="btn btn-danger btn-xs" role="button" class="btn btn-danger btn-xs" data-toggle="modal"
		...
		<div style="width:380px;margin:0 auto;margin-top:10px;">
			<ul class="pagination" style="text-align:center; margin-top:10px;">
				...
				<c:if test="${ pageBean.currPage == 1 }">
					<li class='disabled'>
						<a href="${pageContext.request.contextPath }/findOwnTNews?currPage=1#">&laquo;</span></a>
					</li>
				</c:if>
				<c:if test="${ pageBean.currPage != 1 }">
					<li>
						<a href="${pageContext.request.contextPath }/findOwnTNews?currPage=${ pageBean.currPage-1 }" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a>
					</li>
				</c:if>
				...
				<c:forEach var="i" begin="1" end="${ pageBean.totalPage }">
					<c:if test="${ pageBean.currPage == i }">
						<li class="active"><a href="${pageContext.request.contextPath }/findOwnTNews?currPage=${ i }#">${ i }</a></li>
					</c:if>
					
					<c:if test="${ pageBean.currPage != i }">
						<li><a href="${pageContext.request.contextPath }/findOwnTNews?currPage=${ i }">${ i }</a></li>
					</c:if>	
				</c:forEach>
				...
				<c:if test="${ pageBean.currPage == pageBean.totalPage }">
					<li class='disabled'>
						<a href="${pageContext.request.contextPath }/findOwnTNews?currPage=${ pageBean.totalPage }#" aria-label="Next">
				...
				<c:if test="${ pageBean.currPage != pageBean.totalPage }">
					<li>
						<a href="${pageContext.request.contextPath }/findOwnTNews?currPage=${ pageBean.currPage + 1 }" aria-label="Next">
							<span aria-hidden="true">&raquo;</span>
						</a>
					</li>
				</c:if>
...
<%-- 					<a href="${pageContext.request.contextPath }/deleteCNews?" id="deleteCNewsBt" class="btn btn-danger" role="button" >删除</a>  
 ...
 --%>				
				...
				<button type="button" class="btn btn-default" data-dismiss="modal">算了</button>
 				    <button class="btn btn-danger" onclick="deleteTNews()" id="deleteDiary" type="button">确定删除</button> 
					<nav id="page"> </nav>
				</div>
			</div>
		</div>
