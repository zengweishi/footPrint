  ...
  <body>
		  ...
		  <div style="margin-top: 20px;margin-left: 20px">
		    ...
		    <!-- Indicates a successful or positive action -->
		 		...
		 		<button type="button" value="submit" id="release" onclick="doPost('${pageContext.request.contextPath }/updateTNews')"  name="release" class="btn btn-success">更新文章</button>
 ...
 <%-- <a href="javascript:void(0)"  id="release" onclick="doPost('${pageContext.request.contextPath }/writeCNews')" class="btn btn-primary" role="button" class="btn btn-primary btn-sm" data-toggle="modal"
    ...
    <script type="text/javascript">
       ...
       // 下面两个配置，使用其中一个即可显示“上传图片”的tab。但是两者不要同时使用！！！
	    ...
	    editor.customConfig.uploadImgServer = '${pageContext.request.contextPath}/imgTNewsUpload';
      ...
      //配置onfocus函数之后，用户点击富文本区域会触发onfocus函数执行。
		...
		var updateTNews="<%=request.getAttribute("updateTNews")%>"; 
		document.getElementById("newsTitle").value="${updateTNews.newsTitle}";
		//document.getElementById("editor").setContent("${cnews.newsContents}"); 
		//$("#editor").text("${cnews.newsContents}");
		editor.txt.html("${updateTNews.newsContents}");//牛逼啊这个方法，将数据回显到编辑器
		};
      
	   
		function doPost(url){
			//alert(diaryPermission);
			var newsContents = editor.txt.html();
		    var newsTitle = document.getElementById("newsTitle").value;
		  	var id = "${updateTNews.newsId}";
		    
			$.post(url,{'newsContents':newsContents,'newsTitle':newsTitle,'id':id},function(data) {
		    	window.location.href="${pageContext.request.contextPath}/findTNewsArticle?newsId="+id;
			});
