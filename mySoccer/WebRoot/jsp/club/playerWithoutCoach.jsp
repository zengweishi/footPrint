...
<script type="text/javascript">
	...
	function deletePlayer() {
		location.href = "${pageContext.request.contextPath}/releasePlayer?pid="+i;
	}
	

	function setPlayerCoachBt(pid) {
		i = pid;
	}
	var coachId = "0";
	$(function() {
	...
	function setPlayerCoach() {
		document.getElementById('setPlayerCoachForm').action = "${pageContext.request.contextPath}/setCoachForPlayer?pid="+i+"&coachId="+coachId; 
        document.getElementById("setPlayerCoachForm").submit();  
		//location.href = "${pageContext.request.contextPath}/setCoachForPlayer?pid="+i+"&coachId="+coachId;
	}
	function test1() {
		location.href = "${pageContext.request.contextPath}/setCoachForPlayer?coachId="+coachId+"&pid="+i;
	}
...
<body class="gray-bg">
	                ...
	                <div class="contact-box" style="height: 180px">
	                    <a href="${pageContext.request.contextPath}/getplayerInformation?pid=${p.pid}">
	                        <div class="col-sm-4">
	                            ...
	                            <div class="text-center">
		                            <br/>
		                             <a href="javascript:void(0)" onclick="setPlayerCoachBt('${p.pid}')"  class="btn btn-success btn-xs" role="button" class="btn btn-danger btn-xs" data-toggle="modal"
					data-target="#myModal">分配教练</a> 
	                        ...
	                        <div class="col-sm-8">
	                        	 <div class="ibox-tools">
									...
									<a href="javascript:void(0)" onclick="deletePlayerBt('${p.pid}')"  class="btn btn-danger btn-xs" role="button" class="btn btn-danger btn-xs" data-toggle="modal"
					data-target="#myModal2">解除</a> 
  ...
  <div class="modal-dialog " role="document" style="z-index: 2041;">
    <div class="modal-content">
    		...
    		<form id="setPlayerCoachForm" name="Form1" action="" method="post">
					<div class="modal-body">
						<label for="name">教练列表</label>
							<select class="form-control" style="height: 35px" id="coachSelect" name="coachSelect">
							<option>--请选择--</option>
							    <c:forEach var="p" items="${ coachList }">
					                <option value="${ p.coachId }">${ p.coachRealName }</option>
					            </c:forEach> 
							</select>
	 				</div>
					...
					<div class="modal-footer">
						<button type="button" class="btn btn-default" data-dismiss="modal">返回</button>
	 				    <button class="btn btn-success" onclick="test1()" id="setPlayerCoach" type="button">完成</button> 
						<nav id="page"> </nav>
					</div>
			</form>
...
<%-- 					<a href="${pageContext.request.contextPath }/deleteCNews?" id="deleteCNewsBt" class="btn btn-danger" role="button" >删除</a>  
 ...
 --%>				
				...
				<div class="modal-footer">
				<button type="button" class="btn btn-default" data-dismiss="modal">返回</button>
 				    <button class="btn btn-danger" onclick="deletePlayer()" id="deletePlayer" type="button">确定解除</button> 
					<nav id="page"> </nav>
				</div>
			</div>
		</div>
