...
<script type="text/javascript">
	...
	window.onload = function() {
		...
		if(!(id.length===0)) {
			...
			var newsId = "${cnews.newsId}";
			$.post("${pageContext.request.contextPath}/findCollection",{'cnewsId':newsId},function(data) {
				if(data == "1" ) {
		...
		//获取评论
		var cnews="<%=request.getAttribute("cnews")%>"; 
		var cnewsId = "${cnews.newsId}";
		$.post("${pageContext.request.contextPath}/findCommentsByArticleId",{'cnewsId':cnewsId},function(data) {
				...
				html+='<div class="social-feed-box">'
					...
					if((jsonObj[i].userId == clubId)||(jsonObj[i].userId == coachId)||(jsonObj[i].userId == playerId)||(jsonObj[i].userId == touristId)) {
						html+=' <div class="ibox-tools">'
							+'<a href="javascript:void(0)" onclick="deleteComments(\''+jsonObj[i].commentsId+'\',\''+cnewsId+'\')" >删除</a>'
							+'</div>'
					}  
	...
	function change(newsId) {
		...
		if(type == "收藏") {
			$.post("${pageContext.request.contextPath}/addCollection",{'cnewsId':newsId},function(data) {
				document.getElementById('collection').innerText = "取消收藏";
			});
		} else {
			$.post("${pageContext.request.contextPath}/deleteCollection",{'cnewsId':newsId},function(data) {
				document.getElementById('collection').innerText = "收藏";
			});
		}
	}
	
	function addComments(cnewsId) {
		...
		$.post("${pageContext.request.contextPath}/addComments",{'contents':contents,'cnewsId':cnewsId},function(data) {
			if(data == "1") {
	...
	function deleteComments(commentsId,cnewsId) {
                ...
                <div class="ibox">
                    <div class="ibox-content" > 
                     <div class="pull-right">
                     	<c:if test="${not empty existTourist }">
                   			 <a href="javascript:void(0)" onclick="change('${cnews.newsId}')" id="collection"  class="btn btn-default btn-xs" role="button" >收藏</a> 
	                     </c:if>
	                            
	                     	<c:if test="${(not empty existClub)&&(cnews.newsClub.cid==existClub.cid) }">
	                     		<c:if test="${empty pageFlag }">
		                     		<a href="${pageContext.request.contextPath }/updateCNews?newsId=${cnews.newsId}">
                        ...
                        <div class="text-center article-title">
                            <h2>
                                 <strong>   ${cnews.newsTitle }</strong>
                                </h2>
                        </div>
						<div class="small m-b-xs">
						<c:if test="${not empty pageFlag }">
                          <span > by </span> <strong><a href="${pageContext.request.contextPath }/getClubInformation?pageFlag=1&cid=${cnews.newsClub.cid}">${ cnews.newsClub.clubName }</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong> <span><i class="fa fa-clock-o"></i>&nbsp;&nbsp;${ fn:substring(cnews.newsDate,0,19) }</span>
						</c:if>
						<c:if test="${empty pageFlag }">
                          <span > by </span> <strong><a href="${pageContext.request.contextPath }/getClubInformation?cid=${cnews.newsClub.cid}">${ cnews.newsClub.clubName }</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong> <span><i class="fa fa-clock-o"></i>&nbsp;&nbsp;${ fn:substring(cnews.newsDate,0,19) }</span>
						</c:if>
	                         ...
	                         <div class="input-group">
	                            <input type="text" id="contents" name="contents" placeholder="请在此发表您的评论..." class="input form-control">
	                            	<span class="input-group-btn">
	                                        <button type="button" onclick="addComments('${cnews.newsId}')" class="btn btn btn-info btn-sm">发表</button>
	                                </span>
	                        </div>
	                        <br>
                        </c:if>
