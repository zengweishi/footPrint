...
<div class="tem_banner">
  <ul class="slides" style="height:380px;">
    <li style="background:url(${pageContext.request.contextPath }/images/bb.jpg) no-repeat center center; height:380px;"></li>
  </ul>
</div>
      ...
      <!--新闻列表-->
            ...
            <ul>
            	 <c:forEach var="p" items="${ rightList }" varStatus="status">
            	 	<li><a href="${pageContext.request.contextPath }/showCNewsArtical?pageFlag=1&newsId=${p.newsId}" target="_self"><b > ${status.count}</b>${p.newsTitle }</a></li>
            	 </c:forEach>
            </ul>
          </dd>
        </dl>
  ...
  <article>
    <div class="met_article">
      <section class="met_article_head">
       <div >
	      	<h1 ><strong>俱乐部动态</strong></h1>
	      	<h4><strong>查看最新动态，掌握最全面的青训机构动态</strong></h4>
	      </div>
             ...
             <c:forEach var="p" items="${ topList }">
                <div class="ibox">
                    <div class="ibox-content">
                        <a href="${pageContext.request.contextPath }/showCNewsArtical?pageFlag=1&newsId=${p.newsId}" class="btn-link">
                            <h2>${p.newsTitle }
                            &nbsp;
		...
		<div style="width:380px;margin:0 auto;margin-top:10px;">
			<ul class="pagination" style="text-align:center; margin-top:10px;">
				...
				<c:if test="${ pageBean.currPage == 1 }">
					<li class='disabled'>
						<a href="${pageContext.request.contextPath }/findCNewsForHomePage?currPage=1#">&laquo;</span></a>
					</li>
				</c:if>
				<c:if test="${ pageBean.currPage != 1 }">
					<li>
						<a href="${pageContext.request.contextPath }/findCNewsForHomePage?currPage=${ pageBean.currPage-1 }" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a>
					</li>
				</c:if>
				...
				<c:forEach var="i" begin="1" end="${ pageBean.totalPage }">
					<c:if test="${ pageBean.currPage == i }">
						<li class="active"><a href="{pageContext.request.contextPath }/findCNewsForHomePage?currPage=${ i }#">${ i }</a></li>
					</c:if>
					
					<c:if test="${ pageBean.currPage != i }">
						<li><a href="${pageContext.request.contextPath }/findCNewsForHomePage?currPage=${ i }">${ i }</a></li>
					</c:if>	
				</c:forEach>
				...
				<c:if test="${ pageBean.currPage == pageBean.totalPage }">
					<li class='disabled'>
						<a href="${pageContext.request.contextPath }/findCNewsForHomePage?currPage=${ pageBean.totalPage }#" aria-label="Next">
				...
				<c:if test="${ pageBean.currPage != pageBean.totalPage }">
					<li>
						<a href="${pageContext.request.contextPath }/findCNewsForHomePage?currPage=${ pageBean.currPage + 1 }" aria-label="Next">
							<span aria-hidden="true">&raquo;</span>
						</a>
					</li>
				</c:if>
