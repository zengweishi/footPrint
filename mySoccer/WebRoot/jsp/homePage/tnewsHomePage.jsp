...
<div class="tem_banner">
  <ul class="slides" style="height:380px;">
    <li style="background:url(${pageContext.request.contextPath }/images/oo.jpg) no-repeat center center; height:380px;"></li>
  </ul>
</div>
      ...
      <!--新闻列表-->
            ...
            <ul>
            	 <c:forEach var="p" items="${ rightList }" varStatus="status">
            	 	<li><a href="${pageContext.request.contextPath }/findTNewsArticle?pageFlag=1&newsId=${p.newsId}" target="_self"><b > ${status.count}</b>${p.newsTitle }</a></li>
            	 </c:forEach>
            </ul>
          </dd>
        </dl>
  ...
  <article>
    <div class="met_article">
      <section class="met_article_head">
       <div >
	      	<h1 ><strong>其他资讯</strong></h1>
	      	<h4><strong>综合资讯，应有尽有</strong></h4>
	      </div>
             ...
             <c:forEach var="p" items="${ topList }">
                <div class="ibox">
                    <div class="ibox-content">
                        <a href="${pageContext.request.contextPath }/findTNewsArticle?pageFlag=1&newsId=${p.newsId}" class="btn-link">
                            <h2>${p.newsTitle }
                            &nbsp;
                            <button class="btn btn-danger btn-xs" type="button">置顶</button>
                             	</h2> 
                        ...
                        <div class="row">
                             <div class="col-md-6">
                                <button class="btn btn-default btn-xs" type="button">其他资讯</button>
		...
		<div style="width:380px;margin:0 auto;margin-top:10px;">
			<ul class="pagination" style="text-align:center; margin-top:10px;">
				...
				<c:if test="${ pageBean.currPage == 1 }">
					<li class='disabled'>
						<a href="${pageContext.request.contextPath }/findTNewsForHomePage?currPage=1#">&laquo;</span></a>
					</li>
				</c:if>
				<c:if test="${ pageBean.currPage != 1 }">
					<li>
						<a href="${pageContext.request.contextPath }/findTNewsForHomePage?currPage=${ pageBean.currPage-1 }" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a>
					</li>
				</c:if>
				...
				<c:forEach var="i" begin="1" end="${ pageBean.totalPage }">
					<c:if test="${ pageBean.currPage == i }">
						<li class="active"><a href="${pageContext.request.contextPath }/findTNewsForHomePage?currPage=${ i }#">${ i }</a></li>
					</c:if>
					
					<c:if test="${ pageBean.currPage != i }">
						<li><a href="${pageContext.request.contextPath }/findTNewsForHomePage?currPage=${ i }">${ i }</a></li>
					</c:if>	
				</c:forEach>
				...
				<c:if test="${ pageBean.currPage == pageBean.totalPage }">
					<li class='disabled'>
						<a href="${pageContext.request.contextPath }/findTNewsForHomePage?currPage=${ pageBean.totalPage }#" aria-label="Next">
				...
				<c:if test="${ pageBean.currPage != pageBean.totalPage }">
					<li>
						<a href="${pageContext.request.contextPath }/findTNewsForHomePage?currPage=${ pageBean.currPage + 1 }" aria-label="Next">
							<span aria-hidden="true">&raquo;</span>
						</a>
					</li>
				</c:if>
