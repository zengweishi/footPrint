...
<div class="tem_banner">
  <ul class="slides" style="height:380px;">
    <li style="background:url(${pageContext.request.contextPath }/images/dd.jpg) no-repeat center center; height:380px;"></li>
  </ul>
</div>
      ...
      <!--新闻列表-->
            ...
            <ul>
              <c:forEach var="p" items="${ rightList }" varStatus="status">
            	 	<li><a href="${pageContext.request.contextPath }/findDiaryArtical?pageFlag=1&diaryId=${p.diaryId}" target="_self"><b > ${status.count}</b>${p.diaryTitle }</a></li>
              </c:forEach>
            </ul>
          </dd>
        </dl>
  ...
  <article>
    <div class="met_article">
      <section class="met_article_head">
        <div >
	      	<h1 ><strong>球员日记</strong></h1>
	      	<h4><strong>查看球员心得，一览球员成长</strong></h4>
	      </div>
               ...
               <c:forEach var="p" items="${topList }">
                <div class="ibox">
                    <div class="ibox-content">
                        <a href="${pageContext.request.contextPath }/findDiaryArtical?pageFlag=1&diaryId=${p.diaryId}" class="btn-link">
                            <h2>${p.diaryTitle } &nbsp;<button class="btn btn-danger btn-xs" type="button">置顶</button></h2> 
                        </a>
                        ...
                        <div class="row">
                             <div class="col-md-6">
                                ...
                                <c:if test="${p.diaryPermission==1}">
                               		 <button class="btn btn-warning btn-xs" type="button">仅自己可见</button>
                                </c:if>
                                 <c:if test="${p.diaryPermission==2}">
                               		 <button class="btn btn-primary btn-xs" type="button">仅教练队友可见</button>
                                </c:if>
		...
		<div style="width:380px;margin:0 auto;margin-top:10px;">
			<ul class="pagination" style="text-align:center; margin-top:10px;">
				...
				<c:if test="${ pageBean.currPage == 1 }">
					<li class='disabled'>
						<a href="${pageContext.request.contextPath }/findDiaryForHomePage?currPage=1#">&laquo;</span></a>
					</li>
				</c:if>
				<c:if test="${ pageBean.currPage != 1 }">
					<li>
						<a href="${pageContext.request.contextPath }/findDiaryForHomePage?currPage=${ pageBean.currPage-1 }" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a>
					</li>
				</c:if>
				...
				<c:forEach var="i" begin="1" end="${ pageBean.totalPage }">
					<c:if test="${ pageBean.currPage == i }">
						<li class="active"><a href="{pageContext.request.contextPath }/findDiaryForHomePage?currPage=${ i }#">${ i }</a></li>
					</c:if>
					
					<c:if test="${ pageBean.currPage != i }">
						<li><a href="${pageContext.request.contextPath }/findDiaryForHomePage?currPage=${ i }">${ i }</a></li>
					</c:if>	
				</c:forEach>
				...
				<c:if test="${ pageBean.currPage == pageBean.totalPage }">
					<li class='disabled'>
						<a href="${pageContext.request.contextPath }/findDiaryForHomePage?currPage=${ pageBean.totalPage }#" aria-label="Next">
				...
				<c:if test="${ pageBean.currPage != pageBean.totalPage }">
					<li>
						<a href="${pageContext.request.contextPath }/findDiaryForHomePage?currPage=${ pageBean.currPage + 1 }" aria-label="Next">
							<span aria-hidden="true">&raquo;</span>
						</a>
					</li>
				</c:if>
