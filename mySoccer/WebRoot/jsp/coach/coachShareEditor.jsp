...
<!-- <style type="text/css">
  ...
  <body>
		  ...
		  <div class="input-group">
			  ...
			  <input type="text" id="shareTitle" name="shareTitle" class="form-control" placeholder="请输入标题..." aria-describedby="sizing-addon2">
		  </div>
		  ...
		  <div style="margin-top: 20px;margin-left: 20px">
		    ...
		    <!-- Indicates a successful or positive action -->
		 	<button type="button" value="submit" id="release" onclick="doPost('${pageContext.request.contextPath }/writeShare')"  name="release" class="btn btn-success">发布新分享</button>
		  <%-- </c:if> --%>
  ...
  <div class="modal-dialog modal-sm" role="document" style="z-index: 2041;">
    ...
    <script type="text/javascript">
       ...
       // 下面两个配置，使用其中一个即可显示“上传图片”的tab。但是两者不要同时使用！！！
	    ...
	    editor.customConfig.uploadImgServer = '${pageContext.request.contextPath}/imgShareUpload';
      ...
      //配置onfocus函数之后，用户点击富文本区域会触发onfocus函数执行。
       ...
       <%--  window.onload = function(){
		...
		var share="<%=session.getAttribute("share")%>"; 
		document.getElementById("shareTitle").value="${share.shareTitle}";
		//document.getElementById("editor").setContent("${cnews.newsContents}"); 
		//$("#editor").text("${cnews.newsContents}");
		editor.txt.html("${share.shareContents}");//牛逼啊这个方法，将数据回显到编辑器
		}; --%>
		...
		function doPost(url){
			...
			var i;
			 $.post(url, {'shareContents':shareContents, 'shareTitle':shareTitle,'id':id},function(data) {
			// i = data;
			  /* if(i=="1") { */
    	window.location.href="${pageContext.request.contextPath}/showAllShare?currPage=1";
		...
		//window.location.href="${pageContext.request.contextPath}/showCNewsArtical?newsId="+${cnews.newsId};
			window.location.href="${pageContext.request.contextPath}/showShareArtical?shareId=${share.shareId}" ;
			 }  */
    });
