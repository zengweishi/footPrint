...
<script type="text/javascript">
	var i = "0";
	function deleteTouristBt(tId) {
		i = tId;
	}
	function deleteTourist() {
		location.href = "${pageContext.request.contextPath}/deleteTouristByAdmin?tId="+i;
	}
</script>
    ...
    <div class="row">
							...
							<table class="table table-hover">
								...
								<c:forEach var="p" items="${ pageBean.list }" varStatus="status">
										<tr style="height: 50px" onmouseover="this.style.backgroundColor = 'white'"
											...
											<td style="CURSOR: hand;"  align="center">
												<img alt="image" style="width: 40px;height: 40px; " class="img-circle m-t-xs img-responsive"  src="${p.tPhoto}">
											</td>
											<td style="CURSOR: hand;"  align="center"
												>
												${ p.tEmail }
											...
											<td align="center" >
	 												...
	 												<a href="javascript:void(0)" onclick="deleteTouristBt('${p.tId}')"  class="btn btn-danger btn-sm" role="button" class="btn btn-danger btn-xs" data-toggle="modal"
		...
		<div style="width:380px;margin:0 auto;margin-top:10px;">
			<ul class="pagination" style="text-align:center; margin-top:10px;">
				...
				<c:if test="${ pageBean.currPage == 1 }">
					<li class='disabled'>
						<a href="${pageContext.request.contextPath }/findTouristByAdmin?currPage=1#">&laquo;</span></a>
					</li>
				</c:if>
				<c:if test="${ pageBean.currPage != 1 }">
					<li>
						<a href="${pageContext.request.contextPath }/findTouristByAdmin?currPage=${ pageBean.currPage-1 }" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a>
					</li>
				</c:if>
				...
				<c:forEach var="i" begin="1" end="${ pageBean.totalPage }">
					<c:if test="${ pageBean.currPage == i }">
						<li class="active"><a href="${pageContext.request.contextPath }/findTouristByAdmin?currPage=${ i }#">${ i }</a></li>
					</c:if>
					
					<c:if test="${ pageBean.currPage != i }">
						<li><a href="${pageContext.request.contextPath }/findTouristByAdmin?currPage=${ i }">${ i }</a></li>
					</c:if>	
				</c:forEach>
				...
				<c:if test="${ pageBean.currPage == pageBean.totalPage }">
					<li class='disabled'>
						<a href="${pageContext.request.contextPath }/findTouristByAdmin?currPage=${ pageBean.totalPage }#" aria-label="Next">
				...
				<c:if test="${ pageBean.currPage != pageBean.totalPage }">
					<li>
						<a href="${pageContext.request.contextPath }/findTouristByAdmin?currPage=${ pageBean.currPage + 1 }" aria-label="Next">
							<span aria-hidden="true">&raquo;</span>
						</a>
					</li>
				</c:if>
    ...
    <div class="modal-content">
				...
				<div class="modal-body">
					<!-- <table class="table table-bordered .table-hover">
					 ...
					 <h3 class="modal-title" id="myModalLabel">请您确认是否删除该游客用户？</h3>
<%-- 					<a href="${pageContext.request.contextPath }/deleteCNews?" id="deleteCNewsBt" class="btn btn-danger" role="button" >删除</a>  
 ...
 --%>				
				...
				<button type="button" class="btn btn-default" data-dismiss="modal">返回</button>
 				    <button class="btn btn-danger" onclick="deleteTourist()" id="deleteTourist" type="button">确定删除</button> 
					<nav id="page"> </nav>
				</div>
			</div>
		</div>
