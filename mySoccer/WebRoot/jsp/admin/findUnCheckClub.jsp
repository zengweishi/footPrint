...
<script type="text/javascript">
	...
	function deleteClub() {
		location.href = "${pageContext.request.contextPath}/refuseClub?cid="+i;
	}
	function acceptClubBt(cid) {
		i = cid;
	}
	function acceptClub() {
		location.href = "${pageContext.request.contextPath}/checkClub?cid="+i;
	}
</script>
    ...
    <div class="row">
							...
							<table class="table table-hover">
								...
								<c:forEach var="p" items="${ pageBean.list }" varStatus="status">
										<tr style="height: 50px" onmouseover="this.style.backgroundColor = 'white'"
											...
											<td align="center" >
												<a href="javascript:void(0)" onclick="acceptClubBt('${p.cid}')"  class="btn btn-success btn-sm" role="button" class="btn btn-success btn-xs" data-toggle="modal"
						data-target="#myModal">通过</a> 
		...
		<div style="width:380px;margin:0 auto;margin-top:10px;">
			<ul class="pagination" style="text-align:center; margin-top:10px;">
				...
				<c:if test="${ pageBean.currPage == 1 }">
					<li class='disabled'>
						<a href="${pageContext.request.contextPath }/findUnCheckClubByAdmin?currPage=1#">&laquo;</span></a>
					</li>
				</c:if>
				<c:if test="${ pageBean.currPage != 1 }">
					<li>
						<a href="${pageContext.request.contextPath }/findUnCheckClubByAdmin?currPage=${ pageBean.currPage-1 }" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a>
					</li>
				</c:if>
				...
				<c:forEach var="i" begin="1" end="${ pageBean.totalPage }">
					<c:if test="${ pageBean.currPage == i }">
						<li class="active"><a href="${pageContext.request.contextPath }/findUnCheckClubByAdmin?currPage=${ i }#">${ i }</a></li>
					</c:if>
					
					<c:if test="${ pageBean.currPage != i }">
						<li><a href="${pageContext.request.contextPath }/findUnCheckClubByAdmin?currPage=${ i }">${ i }</a></li>
					</c:if>	
				</c:forEach>
				...
				<c:if test="${ pageBean.currPage == pageBean.totalPage }">
					<li class='disabled'>
						<a href="${pageContext.request.contextPath }/findUnCheckClubByAdmin?currPage=${ pageBean.totalPage }#" aria-label="Next">
				...
				<c:if test="${ pageBean.currPage != pageBean.totalPage }">
					<li>
						<a href="${pageContext.request.contextPath }/findUnCheckClubByAdmin?currPage=${ pageBean.currPage + 1 }" aria-label="Next">
							<span aria-hidden="true">&raquo;</span>
						</a>
					</li>
				</c:if>
    ...
    <div class="modal-content">
				...
				<div class="modal-body">
					<!-- <table class="table table-bordered .table-hover">
					 ...
					 <h3 class="modal-title" id="myModalLabel">请您确认是否激活该俱乐部？</h3>
<%-- 					<a href="${pageContext.request.contextPath }/deleteCNews?" id="deleteCNewsBt" class="btn btn-danger" role="button" >删除</a>  
 ...
 --%>				
				...
				<button type="button" class="btn btn-default" data-dismiss="modal">返回</button>
 				    <button class="btn btn-success" onclick="acceptClub()" id="acceptClub" type="button">确定激活</button> 
					<nav id="page"> </nav>
				</div>
			</div>
		</div>
    ...
    <div class="modal-content">
				...
				<div class="modal-body">
					<!-- <table class="table table-bordered .table-hover">
					 ...
					 <h3 class="modal-title" id="myModalLabel">请您确认是否拒绝该俱乐部？</h3>
<%-- 					<a href="${pageContext.request.contextPath }/deleteCNews?" id="deleteCNewsBt" class="btn btn-danger" role="button" >删除</a>  
 ...
 --%>				
				...
				<div class="modal-footer">
				<button type="button" class="btn btn-default" data-dismiss="modal">返回</button>
 				    <button class="btn btn-danger" onclick="deleteClub()" id="deleteClub" type="button">确定拒绝</button> 
					<nav id="page"> </nav>
				</div>
			</div>
		</div>
