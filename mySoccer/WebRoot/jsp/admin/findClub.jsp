...
<script type="text/javascript">
	var i = "0";
	function deleteClubBt(cid) {
		i = cid;
	}
	function deleteClub() {
		location.href = "${pageContext.request.contextPath}/deleteClub?cid="+i;
	}
	
	<%-- window.onload = function() {
		 <%
		 List<Club> list = (List)request.getAttribute("clubList");
		 Iterator it1 = list.iterator();
		 Integer temp;
		 while(it1.hasNext())
		 {
		
		 Club club = (Club)it1.next();
		 temp = (Integer)club.getClubMainPage();
		 %>
		 var flag = "<%=temp%>";
	
			if(flag == 1) {
				document.getElementById('setPage').innerText = "取消首页";
			} else {
				document.getElementById('setPage').innerText = "上首页";
			}
	...
	function setMainPage(cid) {
		var bt = "setPage"+cid; //各个button根据ID来命名
		if(document.getElementById(bt).innerText == "上首页") {
			$.post("${pageContext.request.contextPath}/setMainPage",{'cid':cid},function(data){
				document.getElementById(bt).className = "btn btn-info btn-sm"; 
				document.getElementById(bt).innerText = "取消首页";
			});	
		} else {
			$.post("${pageContext.request.contextPath}/downMainPage",{'cid':cid},function(data){
	...
	function downMainPage(cid) {
		var bt = "downPage"+cid; //各个button根据ID来命名
		if(document.getElementById(bt).innerText == "上首页") {
			$.post("${pageContext.request.contextPath}/setMainPage",{'cid':cid},function(data){
				document.getElementById(bt).className = "btn btn-info btn-sm"; 
				document.getElementById(bt).innerText = "取消首页";
    ...
    <div class="row">
								...
								<c:forEach var="p" items="${ pageBean.list }" varStatus="status">
										<tr style="height: 50px" onmouseover="this.style.backgroundColor = 'white'"
											...
											<td align="center" width=10%>
												...
												<c:if test="${p.clubMainPage ne 1 }">
													<a href="javascript:void(0)" onclick="setMainPage('${p.cid}')" >
														<button type="button" id="setPage${p.cid}" name="setPage" class="btn btn-success btn-sm">
			                                                                                                                                                 上首页
												       </button>
	 												</a>
	 											</c:if>
	 											
	 											<c:if test="${p.clubMainPage == 1 }">
													<a href="javascript:void(0)" onclick="downMainPage('${p.cid}')" >
														<button type="button" id="downPage${p.cid}" name="downPage" class="btn btn-info btn-sm">
																取消首页
														</button>
													</a> 
												</c:if>	
											...
											<td align="center">
												 <c:if test="${ p.clubCoachFlag==0 }">
	 												<a href="javascript:void(0)" onclick="deleteClubBt('${p.cid}')"  class="btn btn-danger btn-sm" role="button" class="btn btn-danger btn-xs" data-toggle="modal"
						data-target="#myModal">删除</a> 
												 </c:if> 
												  <c:if test="${ p.clubCoachFlag ne 0 }">
		...
		<div style="width:380px;margin:0 auto;margin-top:10px;">
			<ul class="pagination" style="text-align:center; margin-top:10px;">
				...
				<c:if test="${ pageBean.currPage == 1 }">
					<li class='disabled'>
						<a href="${pageContext.request.contextPath }/findCheckClubByAdmin?currPage=1#">&laquo;</span></a>
					</li>
				</c:if>
				<c:if test="${ pageBean.currPage != 1 }">
					<li>
						<a href="${pageContext.request.contextPath }/findCheckClubByAdmin?currPage=${ pageBean.currPage-1 }" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a>
					</li>
				</c:if>
				...
				<c:forEach var="i" begin="1" end="${ pageBean.totalPage }">
					<c:if test="${ pageBean.currPage == i }">
						<li class="active"><a href="${pageContext.request.contextPath }/findCheckClubByAdmin?currPage=${ i }#">${ i }</a></li>
					</c:if>
					
					<c:if test="${ pageBean.currPage != i }">
						<li><a href="${pageContext.request.contextPath }/findCheckClubByAdmin?currPage=${ i }">${ i }</a></li>
					</c:if>	
				</c:forEach>
				...
				<c:if test="${ pageBean.currPage == pageBean.totalPage }">
					<li class='disabled'>
						<a href="${pageContext.request.contextPath }/findCheckClubByAdmin?currPage=${ pageBean.totalPage }#" aria-label="Next">
				...
				<c:if test="${ pageBean.currPage != pageBean.totalPage }">
					<li>
						<a href="${pageContext.request.contextPath }/findCheckClubByAdmin?currPage=${ pageBean.currPage + 1 }" aria-label="Next">
							<span aria-hidden="true">&raquo;</span>
						</a>
					</li>
				</c:if>
    ...
    <div class="modal-content">
				...
				<div class="modal-body">
					<!-- <table class="table table-bordered .table-hover">
					 ...
					 <h3 class="modal-title" id="myModalLabel">请您确认是否删除该俱乐部？</h3>
<%-- 					<a href="${pageContext.request.contextPath }/deleteCNews?" id="deleteCNewsBt" class="btn btn-danger" role="button" >删除</a>  
 ...
 --%>				
				...
				<button type="button" class="btn btn-default" data-dismiss="modal">返回</button>
 				    <button class="btn btn-danger" onclick="deleteClub()" id="deleteClub" type="button">确定删除</button> 
					<nav id="page"> </nav>
				</div>
			</div>
		</div>
