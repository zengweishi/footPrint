...
<script type="text/javascript">
	var i = "0";
	function deleteShareBt(shareId) {
		i = shareId;
	}
	function deleteShare() {
		location.href = "${pageContext.request.contextPath}/deleteShareByAdmin?shareId="+i;
	}
	
	
	function setTop(shareId) {
		var bt = "setTop"+shareId; //各个button根据ID来命名
		if(document.getElementById(bt).innerText == "置顶") {
			$.post("${pageContext.request.contextPath}/setTop",{'shareId':shareId},function(data){
				document.getElementById(bt).className = "btn btn-info btn-sm"; 
				document.getElementById(bt).innerText = "取消置顶";
			});	
		} else {
			$.post("${pageContext.request.contextPath}/downTop",{'shareId':shareId},function(data){
	...
	function downTop(shareId) {
		var bt = "downTop"+shareId; //各个button根据ID来命名
		if(document.getElementById(bt).innerText == "置顶") {
			$.post("${pageContext.request.contextPath}/setTop",{'shareId':shareId},function(data){
				document.getElementById(bt).className = "btn btn-info btn-sm"; 
				document.getElementById(bt).innerText = "取消置顶";
    ...
    <div class="row">
							...
							<table class="table table-hover">
								...
								<c:forEach var="p" items="${ pageBean.list }" varStatus="status">
										<tr style="height: 50px" onmouseover="this.style.backgroundColor = 'white'"
											...
											<td style="CURSOR: hand;"  align="center"
												>
												${ fn:substring(p.shareDate,0,19) }
											</td>
											<td style="CURSOR: hand;"  align="center"
												>
												${ p.shareCoach.coachRealName }
											</td>
											<td style="CURSOR: hand;"  align="center"
												>
												${ p.shareCoach.coachName }
											</td>
											
											<td align="center" width=10%>
												<c:if test="${p.shareTopFlag ne 1 }">
											
													<a href="javascript:void(0)" onclick="setTop('${p.shareId}')" >
														<button type="button" id="setTop${p.shareId}" name="setTop" class="btn btn-success btn-sm">
			                                                                                                                                                 置顶
												       </button>
	 												</a>
	 											</c:if>
	 											
	 											<c:if test="${p.shareTopFlag == 1 }">
													<a href="javascript:void(0)" onclick="downTop('${p.shareId}')" >
														<button type="button" id="downTop${p.shareId}" name="downTop" class="btn btn-info btn-sm">
															取消置顶
														</button>
													</a> 
												</c:if>	
											...
											<td align="center" >
												...
												<a href="${pageContext.request.contextPath }/showShareArtical?shareId=${p.shareId}" class="btn btn-primary btn-sm" role="button" >查看</a> 
											</td>
											
												<td align="center" >
	 												<a href="javascript:void(0)" onclick="deleteShareBt('${p.shareId}')"  class="btn btn-danger btn-sm" role="button" class="btn btn-danger btn-xs" data-toggle="modal"
		...
		<div style="width:380px;margin:0 auto;margin-top:10px;">
			<ul class="pagination" style="text-align:center; margin-top:10px;">
				...
				<c:if test="${ pageBean.currPage == 1 }">
					<li class='disabled'>
						<a href="${pageContext.request.contextPath }/findClubNewsByAdmin?currPage=1#">&laquo;</span></a>
					</li>
				</c:if>
				<c:if test="${ pageBean.currPage != 1 }">
					<li>
						<a href="${pageContext.request.contextPath }/findClubNewsByAdmin?currPage=${ pageBean.currPage-1 }" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a>
					</li>
				</c:if>
				...
				<c:forEach var="i" begin="1" end="${ pageBean.totalPage }">
					<c:if test="${ pageBean.currPage == i }">
						<li class="active"><a href="${pageContext.request.contextPath }/findClubNewsByAdmin?currPage=${ i }#">${ i }</a></li>
					</c:if>
					
					<c:if test="${ pageBean.currPage != i }">
						<li><a href="${pageContext.request.contextPath }/findClubNewsByAdmin?currPage=${ i }">${ i }</a></li>
					</c:if>	
				</c:forEach>
				...
				<c:if test="${ pageBean.currPage == pageBean.totalPage }">
					<li class='disabled'>
						<a href="${pageContext.request.contextPath }/findClubNewsByAdmin?currPage=${ pageBean.totalPage }#" aria-label="Next">
				...
				<c:if test="${ pageBean.currPage != pageBean.totalPage }">
					<li>
						<a href="${pageContext.request.contextPath }/findClubNewsByAdmin?currPage=${ pageBean.currPage + 1 }" aria-label="Next">
							<span aria-hidden="true">&raquo;</span>
						</a>
					</li>
				</c:if>
...
<%-- 					<a href="${pageContext.request.contextPath }/deleteCNews?" id="deleteCNewsBt" class="btn btn-danger" role="button" >删除</a>  
 ...
 --%>				
				...
				<button type="button" class="btn btn-default" data-dismiss="modal">返回</button>
 				    <button class="btn btn-danger" onclick="deleteShare()" id="deleteShare" type="button">确定删除</button> 
					<nav id="page"> </nav>
				</div>
			</div>
		</div>
