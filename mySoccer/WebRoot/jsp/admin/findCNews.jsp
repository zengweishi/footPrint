...
<script type="text/javascript">
	var i = "0";
	function deleteCNewsBt(newsId) {
		i = newsId;
	}
	function deleteCNews() {
		location.href = "${pageContext.request.contextPath}/deleteCNewsByAdmin?newsId="+i;
	}
	
	
	
	function setTop(cnewsId) {
		var bt = "setTop"+cnewsId; //各个button根据ID来命名
		if(document.getElementById(bt).innerText == "置顶") {
			$.post("${pageContext.request.contextPath}/setTop",{'cnewsId':cnewsId},function(data){
				document.getElementById(bt).className = "btn btn-info btn-sm"; 
				document.getElementById(bt).innerText = "取消置顶";
			});	
		} else {
			$.post("${pageContext.request.contextPath}/downTop",{'cnewsId':cnewsId},function(data){
	...
	function downTop(cnewsId) {
		var bt = "downTop"+cnewsId; //各个button根据ID来命名
		if(document.getElementById(bt).innerText == "置顶") {
			$.post("${pageContext.request.contextPath}/setTop",{'cnewsId':cnewsId},function(data){
				document.getElementById(bt).className = "btn btn-info btn-sm"; 
				document.getElementById(bt).innerText = "取消置顶";
								...
								<c:forEach var="p" items="${ pageBean.list }" varStatus="status">
										<tr style="height: 50px" onmouseover="this.style.backgroundColor = 'white'"
											...
											<td align="center" width=10% >
												<c:if test="${p.newsTopFlag ne 1 }">
											
													<a href="javascript:void(0)" onclick="setTop('${p.newsId}')" >
														<button type="button" id="setTop${p.newsId}" name="setTop" class="btn btn-success btn-sm">
			                                                                                                                                                 置顶
												       </button>
	 												</a>
	 											</c:if>
	 											
	 											<c:if test="${p.newsTopFlag == 1 }">
														<a href="javascript:void(0)" onclick="downTop('${p.newsId}')" >
															<button type="button" id="downTop${p.newsId}" name="downTop" class="btn btn-info btn-sm">
																取消置顶
															</button>
														</a> 
												</c:if>	
											...
											<td align="center">
												...
												<a href="${pageContext.request.contextPath }/showCNewsArtical?newsId=${p.newsId}" class="btn btn-primary btn-sm" role="button" >查看</a> 
<!-- 											</td>
 -->											
											...
											<td align="center">
	 												<a href="javascript:void(0)" onclick="deleteCNewsBt('${p.newsId}')"  class="btn btn-danger btn-sm" role="button" class="btn btn-danger btn-xs" data-toggle="modal"
...
<%-- 					<a href="${pageContext.request.contextPath }/deleteCNews?" id="deleteCNewsBt" class="btn btn-danger" role="button" >删除</a>  
 ...
 --%>				
				...
				<button type="button" class="btn btn-default" data-dismiss="modal">返回</button>
 				    <button class="btn btn-danger" onclick="deleteCNews()" id="deleteClub" type="button">确定删除</button> 
					<nav id="page"> </nav>
				</div>
			</div>
		</div>
