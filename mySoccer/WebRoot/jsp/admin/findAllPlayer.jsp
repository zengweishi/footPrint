...
<script type="text/javascript">
	var i = "0";
	function deletePlayerBt(pid) {
		i = pid;
	}
	function deletePlayer() {
		location.href = "${pageContext.request.contextPath}/deletePlayerByAdmin?pid="+i;
	}
	
	
	
	function setMainPage(pid) {
		var bt = "setPage"+pid; //各个button根据ID来命名
		if(document.getElementById(bt).innerText == "上首页") {
			$.post("${pageContext.request.contextPath}/setPlayerMainPage",{'pid':pid},function(data){
				document.getElementById(bt).className = "btn btn-info btn-sm"; 
				document.getElementById(bt).innerText = "取消首页";
			});	
		} else {
			$.post("${pageContext.request.contextPath}/downPlayerMainPage",{'pid':pid},function(data){
	...
	function downMainPage(pid) {
		var bt = "downPage"+pid; //各个button根据ID来命名
		if(document.getElementById(bt).innerText == "上首页") {
			$.post("${pageContext.request.contextPath}/setPlayerMainPage",{'pid':pid},function(data){
				document.getElementById(bt).className = "btn btn-info btn-sm"; 
				document.getElementById(bt).innerText = "取消首页";
    ...
    <div class="row">
							...
							<!-- <table cellspacing="0" cellpadding="1" 
								...
								<c:forEach var="p" items="${ pageBean.list }" varStatus="status">
										<tr style="height: 50px" onmouseover="this.style.backgroundColor = 'white'"
											...
											<td style="CURSOR: hand;"  align="center">
												<img alt="image" style="width: 40px;height: 40px; " class="img-circle m-t-xs img-responsive"  src="${p.pPhoto}">
											</td>
											<td style="CURSOR: hand;"  align="center"
												>
												${ p.pNumber }
											...
											<td align="center" width=10%>
												<c:if test="${p.playerMainPage ne 1 }">
													<a href="javascript:void(0)" onclick="setMainPage('${p.pid}')" >
														<button type="button" id="setPage${p.pid}" name="setPage" class="btn btn-success btn-sm">
			                                                                                                                                                 上首页
												       </button>
	 												</a>
	 											</c:if>
	 											
	 											<c:if test="${p.playerMainPage == 1 }">
													<a href="javascript:void(0)" onclick="downMainPage('${p.pid}')" >
														<button type="button" id="downPage${p.pid}" name="downPage" class="btn btn-info btn-sm">
																取消首页
														</button>
													</a> 
												</c:if>	
											...
											<td align="center" >
												<c:if test="${empty p.pClubId }">
	 												<a href="javascript:void(0)" onclick="deletePlayerBt('${p.pid}')"  class="btn btn-danger btn-sm" role="button" class="btn btn-danger btn-xs" data-toggle="modal"
						data-target="#myModal">删除</a> 
												</c:if>
												<c:if test="${ not empty p.pClubId }">
		...
		<div style="width:380px;margin:0 auto;margin-top:10px;">
			<ul class="pagination" style="text-align:center; margin-top:10px;">
				...
				<c:if test="${ pageBean.currPage == 1 }">
					<li class='disabled'>
						<a href="${pageContext.request.contextPath }/findAllPlayer?currPage=1#">&laquo;</span></a>
					</li>
				</c:if>
				<c:if test="${ pageBean.currPage != 1 }">
					<li>
						<a href="${pageContext.request.contextPath }/findAllPlayer?currPage=${ pageBean.currPage-1 }" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a>
					</li>
				</c:if>
				...
				<c:forEach var="i" begin="1" end="${ pageBean.totalPage }">
					<c:if test="${ pageBean.currPage == i }">
						<li class="active"><a href="${pageContext.request.contextPath }/findAllPlayer?currPage=${ i }#">${ i }</a></li>
					</c:if>
					
					<c:if test="${ pageBean.currPage != i }">
						<li><a href="${pageContext.request.contextPath }/findAllPlayer?currPage=${ i }">${ i }</a></li>
					</c:if>	
				</c:forEach>
				...
				<c:if test="${ pageBean.currPage == pageBean.totalPage }">
					<li class='disabled'>
						<a href="${pageContext.request.contextPath }/findAllPlayer?currPage=${ pageBean.totalPage }#" aria-label="Next">
				...
				<c:if test="${ pageBean.currPage != pageBean.totalPage }">
					<li>
						<a href="${pageContext.request.contextPath }/findAllPlayer?currPage=${ pageBean.currPage + 1 }" aria-label="Next">
							<span aria-hidden="true">&raquo;</span>
						</a>
					</li>
				</c:if>
    ...
    <div class="modal-content">
				...
				<div class="modal-body">
					<!-- <table class="table table-bordered .table-hover">
					 ...
					 <h3 class="modal-title" id="myModalLabel">请您确认是否删除该球员？</h3>
<%-- 					<a href="${pageContext.request.contextPath }/deleteCNews?" id="deleteCNewsBt" class="btn btn-danger" role="button" >删除</a>  
 ...
 --%>				
				...
				<button type="button" class="btn btn-default" data-dismiss="modal">返回</button>
 				    <button class="btn btn-danger" onclick="deletePlayer()" id="deletePlayer" type="button">确定删除</button> 
					<nav id="page"> </nav>
				</div>
			</div>
		</div>
